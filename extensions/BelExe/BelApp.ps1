# Load necessary assemblies for GUI and system drawing
...
function Initiate-Call {
    try {
        ...
        # Extract extension and phone number
        $extension = $extensionTextBox.Text
        $phoneNumber = '0' + ($phoneTextBox.Text -replace '[^\d]', '')  # Remove non-digit characters and add leading zero

        # Path to the executable for initiating a call
        $path = 'C:\Program Files (x86)\FileMaker\FileMaker Pro 14\Bel.exe'

        # Start the process to initiate the call
        Start-Process -FilePath $path -ArgumentList $extension, $phoneNumber -NoNewWindow

        # Save the extension when a call is initiated
        Save-Extension $extension
        [System.Windows.MessageBox]::Show("Call initiated.")
    } catch {
