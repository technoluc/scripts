...
function advanced_settings_cloudinit() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname (leave blank for default: \"$DEFAULT_HOSTNAME\")" 8 58 --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="$DEFAULT_HOSTNAME"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
  ...
  if USERNAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Username (Default: \"$DEFAULT_USERNAME\")" 8 58 --title "USERNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a \"$OS\" VM?" --no-button Do-Over 10 58); then
    echo -e "${RD}Creating a \"$DEFAULT_USERNAME\" VM using the above advanced settings${CL}"
